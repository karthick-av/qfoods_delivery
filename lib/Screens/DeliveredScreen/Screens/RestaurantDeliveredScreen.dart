...
class RestaurantDeliveredScreen extends StatefulWidget {
  const RestaurantDeliveredScreen({super.key});

  @override
  State<RestaurantDeliveredScreen> createState() => _RestaurantDeliveredScreenState();
}

class _RestaurantDeliveredScreenState extends State<RestaurantDeliveredScreen> {
...
List<OrderModel> orders = [];
  ...
  //  getOrdersByStatusHandler();
     ...
     geDeliveredOrders_Handler();
  super.initState();
  }
Future<void> geDeliveredOrders_Handler() async{
...
try{
    ...
    var response = await http.get(Uri.parse("${ApiServices.delivered_orders}?person_id=${person_id}&page=${current_page}&per_page=${per_page}"));
...
void dispose(){
 ...
 orders = [];
 setState(() {});
         await geDeliveredOrders_Handler();
      },
